...
app.post("/api/orders",(request, response)=>{  
    ...
    if(!err){
        con.query(command,(err, result)=>{
        if(!err){
            response.send("new order placed placed ");
        }
        else{
            let str_err=JSON.stringify(err);
            console.log(str_err); 
...
app.delete("/api/orders/:id",
            (request, response)=>{ 
                    ...
                    let command1="delete from orderdetails where orderid="+orderid;
                    ...
                    if(!err){
                        con.query(command,(err, result)=>{
                        if(!err){       
                            let command1="delete from orders where orderid="+orderid;
                            con.query(command1,(err, result)=>{
                                if(!err){
                                    response.send("order is cancelled...");
                                }
                                else{
                                    console.log(err);
                                }
